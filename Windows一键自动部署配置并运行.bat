...
:: 使用 Windows curl 极其安全地拉取 node 官方 Windows 部署程序至临时缓存文件夹
:: 移除版本文件名多余的破折号以和官方下载格式匹配
...
:: 自动运行 msi 安装程序。/passive 代表显示最简单的安装进度条，/norestart 预防电脑重启。
:: 使用 start /wait 强力阻塞等待，确保安装完成再进行后续步骤。
